...
<div class="cm-tabs-content">
	<div id="content_tab_new_profile{$id}">
		...
		<div class="form-field">
			...
			{if $id}
				...
				<span class="shift-input">
					...
					{elseif $field.section == "B" || $field.section == "S"}
						{$lang.billing_address}/{$lang.shipping_address}
					{elseif $field.section == "P"}
						{$lang.tspsc_supplier}
					{/if}
				</span>
			{else}
				<select id="elm_field_section" name="field_data[section]">
					...
					<option value="P">{$lang.tspsc_supplier}</option>
				</select>
			{/if}
		</div>
		...
		<div class="form-field">
			...
			<input type="checkbox" name="field_data[{$_show}]" value="Y" {if $field.$_show == "Y"}checked="checked"{/if} id="sw_req_{$_required}" class="cm-switch-availability checkbox" />&nbsp;
